//
//  OfferAdminFilterViewController.swift
//  Domostroy
//
...
final class OfferAdminFilterViewController: UIViewController {
    ...
    private lazy var mainVStackView = {
        ...
        $0.addArrangedSubview(statusRadioGroupVStackView)
        return $0
    }(UIStackView())
    ...
    private lazy var statusRadioButtonGroup = {
        $0.onDidSelect = { [weak self] value in
            self?.output?.setStatus(value)
        }
        return $0
    }(DRadioButtonGroup<OfferAdminFilterViewModel.OfferStatusViewModel>())

    private lazy var statusLabel = {
        $0.font = .systemFont(ofSize: 20, weight: .semibold)
        $0.text = L10n.Localizable.AdminPanel.Offers.Filter.status
        return $0
    }(UILabel())

    private lazy var statusRadioGroupVStackView = {
        $0.axis = .vertical
        return $0
    }(UIStackView())
    ...
    var output: OfferAdminFilterViewOutput?
...
private extension OfferAdminFilterViewController {
...
// MARK: - OfferAdminFilterViewInput

extension OfferAdminFilterViewController: OfferAdminFilterViewInput {
    ...
    func setStatusFilter(
        ...
        statusRadioGroupVStackView.arrangedSubviews.forEach { $0.removeFromSuperview() }
        options.forEach { value in
            let radio = DRadioButton()
            radio.setTitle(value.description)
            statusRadioButtonGroup.add(button: radio, value: value)
            if value == initial {
                statusRadioButtonGroup.select(button: radio)
            }
            statusRadioGroupVStackView.addArrangedSubview(radio)
        }
    }
